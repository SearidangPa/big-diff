...
M.setup_buffer = function(lines, ref_lines)
  ...
  vim.b[buf_id].minidiff_config = { source = require('big-diff.nvim').gen_source.none() }

  require('big-diff.nvim').set_ref_text(buf_id, ref_lines)
...
M.get_hunks = function(buf_id)
  return require("big-diff.nvim").get_buf_data(buf_id).hunks
end
